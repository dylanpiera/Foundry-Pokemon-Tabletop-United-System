...
export class PTUActor extends Actor {
  ...
  async transform() {
    ...
    const alreadyTransformed = selectedActor.effects.find(effect => effect.data.label == "Transform");
    ...
    if (!alreadyTransformed){
      ...
      const targetItems = [...targetActor.items.filter(item => item.type === "move" || item.type === "ability" || item.type === "capability" ).map(item => item.data), transformMove.data];
