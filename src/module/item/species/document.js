...
class PTUSpecies extends PTUItem {
    ...
    getImagePath({ gender = game.i18n.localize("PTU.Male"), shiny = false, extension = game.settings.get("ptu", "defaultPokemonImageExtension"), suffix = "" }={}) {
        ...
        return `${path.endsWith('/') ? "" : "/"}${path}${useName ? this.slug : Handlebars.helpers.lpad(this.system.number, 3, 0)}${femaleTag}${shinyTag}${this.system.form ? ("_" + this.system.form) : ""}${suffix ?? ""}${extension.startsWith('.') ? "" : "."}${extension}`;
    }
